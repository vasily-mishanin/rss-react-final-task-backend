...
const createBoard = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const bodyError = (0, error_service_1.checkBody)(req.body, ['title', 'owner', 'users']);
    if (bodyError) {
        return res.status(400).send((0, error_service_1.createError)(400, "bad request: " + bodyError));
    }
    const { title, owner, users } = req.body;
    try {
        const newBoard = yield boardService.createBoard({ title, owner, users }, guid, initUser);
        res.json(newBoard);
    }
    catch (err) {
...
const updateBoard = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { title, owner, users } = req.body;
    try {
        const updatedBoard = yield boardService.updateBoard(req.params['boardId'], { title, owner, users }, guid, initUser);
